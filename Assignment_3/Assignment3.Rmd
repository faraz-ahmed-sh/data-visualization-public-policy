...
t_caption <- theme(plot.caption=element_text(size=8, face="italic", color="black"))
```

## Graph 1:

```{r graph1, fig.width=8, fig.height=6}
...
ggplot(data=summ_crimes_by_neighborhood, aes(x = Neighborhood, y= freq)) + 
  ...
  labs(title = "What Places in Lahore are Most Crime-Prone?", x = "Neighborhood", y = "Frequency of Crime Incidences", subtitle="Number of crime occurrences in   different neighborhoods in January 2015.", caption = "Source: Punjab Police Department") + t_title + t_axis + t_subtitle + t_caption + 
...
ggplot(by_wday_area_crime_count, aes(x = week_day, y = freq)) + 
  ...
  labs(title = "In Lahore, Be Careful on Weekends", x = "Day of the Week", y = "Total Crime Incidences in January 2015", subtitle="Most neighborhoods of Lahore are crime-prone on Thurs, Fri and Sat during midday and early afternoon in January 2015.", caption = "Source: Punjab Police Department") + 
  t_title + t_axis + t_subtitle + t_caption +
